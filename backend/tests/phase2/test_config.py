...
SONNET_AGENTS = {"sentiment", "synthesis"}
...
class TestSignalWeights:
    def test_weights_sum_to_one(self):
        ...
        assert abs(total - 1.0) < 1e-6, f"Signal weights must sum to 1.0, got {total}"
    ...
    def test_thresholds_in_unit_range(self):
        ...
        for name, val in vars(cfg.synthesis).items():
            if isinstance(val, float):
                assert (
                    0.0 <= val <= 1.0
                ), f"Threshold '{name}' = {val} out of [0, 1] range"
