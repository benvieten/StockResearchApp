...
class TestLowVolScore:
    def test_low_vol_stock_scores_high(self, compute_low_vol, sample_ohlcv_df):
        ...
        low_vol_prices = sample_ohlcv_df["Close"] * (
            1 + np.random.normal(0, 0.002, len(sample_ohlcv_df))
        )
    ...
    def test_high_vol_stock_scores_low(self, compute_low_vol, sample_ohlcv_df):
        high_vol_prices = sample_ohlcv_df["Close"] * (
            1 + np.random.normal(0, 0.04, len(sample_ohlcv_df))
        )
...
class TestReturnZscore:
    ...
    def test_returns_none_with_insufficient_data(self, fn):
        ...
        df = pd.DataFrame(
            {"Close": prices}, index=pd.bdate_range(end="2026-03-18", periods=50)
        )
    ...
    def test_large_up_day_gives_positive_zscore(self, fn, sample_ohlcv_df):
        ...
        new_row = pd.DataFrame(
            {
                "Open": last_price,
                "High": last_price * 1.12,
                "Low": last_price,
                "Close": last_price * 1.10,
                "Volume": 100_000_000,
            },
        ...
        assert (
            result is not None and result > 1.5
...
class TestVolumeRatio:
    ...
    def test_high_volume_day_gives_ratio_above_one(self, fn, sample_ohlcv_df):
        ...
        result = fn(df)
        assert (
            result is not None and result > 5.0
...
class TestBbPercentile:
    ...
    def test_price_at_upper_band_gives_high_percentile(self, fn):
        ...
        result = fn(df)
        assert (
            result is not None and result >= 0.9
